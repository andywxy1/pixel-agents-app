...
Electron desktop app with embedded React webview: pixel art office where AI agents (Claude Code terminals) are animated characters.
...
electron/                     — Electron main process
  ...
  terminalLauncher.ts         — Opens Terminal.app via AppleScript
...
src/                          — Shared backend (Node.js)
  ...
  types.ts                    — Shared interfaces (AgentState, PersistedAgent, ElectronTerminalHandle)
...
**Vocabulary**: Terminal = macOS Terminal.app running Claude. Session = JSONL conversation file. Agent = webview character bound 1:1 to a terminal.

**Main ↔ Webview IPC**: Electron IPC via `ipcMain`/`ipcRenderer`, bridged through `preload.ts`. Key messages: `openClaude`, `agentCreated/Closed`, `focusAgent`, `agentToolStart/Done/Clear`, `agentStatus`, `existingAgents`, `layoutLoaded`, `furnitureAssetsLoaded`, `floorTilesLoaded`, `wallTilesLoaded`, `saveLayout`, `saveAgentSeats`, `exportLayout`, `importLayout`, `settingsLoaded`, `setSoundEnabled`.

**One-agent-per-terminal**: Each "+ Agent" click → new Terminal.app window (`claude --session-id <uuid>`) → immediate agent creation → 1s poll for `<uuid>.jsonl` → file watching starts.

**Auto-detection**: `ensureMultiProjectScan()` watches ALL of `~/.claude/projects/` subdirs. On startup: seeds `knownJsonlFiles` with all existing JSONL (so only NEW sessions auto-spawn characters).
...
**State per agent**: `id, terminalRef, projectDir, jsonlFile, fileOffset, lineBuffer, activeToolIds, activeToolStatuses, activeSubagentToolNames, isWaiting`.

**Persistence**: Agents persisted to `~/.pixel-agents/agents.json`. Agent seats (palette/hueShift/seatId) persisted to `~/.pixel-agents/agent-seats.json`. **Layout persisted to `~/.pixel-agents/layout.json`** (user-level). `layoutPersistence.ts` handles all file I/O: `readLayoutFromFile()`, `writeLayoutToFile()` (atomic via `.tmp` + rename), `migrateAndLoadLayout()` (checks file → falls back to bundled default), `watchLayoutFile()` (hybrid `fs.watch` + 2s polling for cross-window sync). On save, `markOwnWrite()` prevents the watcher from re-reading our own write. On webview ready: `restoreAgents()` matches persisted entries to live terminals. `nextAgentId`/`nextTerminalIndex` advanced past restored values. **Default layout**: When no saved layout file exists, a bundled `default-layout.json` is loaded from `assets/` and written to the file. If that also doesn't exist, `createDefaultLayout()` generates a basic office. To update the default: use App menu → "Export Layout as Default" (writes current layout to `webview-ui/public/assets/default-layout.json`), then rebuild. **Export/Import**: Settings modal offers Export Layout (save dialog → JSON file) and Import Layout (open dialog → validates `version: 1` + `tiles` array → writes to layout file + pushes `layoutLoaded` to webview).
...
**Loading**: `esbuild.electron.js` copies `webview-ui/public/assets/` → `dist/assets/`. Loader checks bundled path first, falls back to workspace root. PNG → pngjs → SpriteData (2D hex array, alpha≥128 = opaque). `loadDefaultLayout()` reads `assets/default-layout.json` (JSON OfficeLayout) as fallback for new installs.

**Asset path resolution**: `app.isPackaged` → `process.resourcesPath/assets/`, dev → `dist/assets/`, fallback → `webview-ui/public/assets/`.
...
- `fs.watch` unreliable on some platforms — always pair with polling backup
...
```
Build: esbuild (electron main + preload) → vite (webview). `npm run start` to build + launch.

Dev mode: `npm run watch:electron-main` + `npm run watch:webview` in separate terminals, then `npm run start:dev`.

Package: `npm run package:mac` → DMG in `release/`.
...
- **Backend**: `src/constants.ts` — timing intervals, display truncation limits, PNG/asset parsing values
...
- `crypto.randomUUID()` works in Node.js
- Terminal `cwd` set via `terminalLauncher.ts` AppleScript
- `/add-dir <path>` grants session access to additional directory

## Key Decisions

- Electron app with BrowserWindow (single-window)
- Webview is separate Vite project with own `node_modules`/`tsconfig`
- IPC abstracted through `vscodeApi.ts` which detects Electron IPC bridge
- Terminal launched via AppleScript → Terminal.app (macOS only currently)
